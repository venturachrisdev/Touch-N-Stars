<template>
  ...
  <div>
    ...
    <input type="range" v-model="framingStore.fov" min="1" max="50" step="1" @input="getTargetPic" />
  </div>
</template>
...
const width = ref(200);  // Breite des verschiebbaren Elements
const height =  ref(200); // Höhe des verschiebbaren Elements
const scaleDegPerPixel = ref(0.004); // Grad pro Pixel  
// -------------------------------
// 1) Container- und FOV-Einstellungen
// -------------------------------
const containerSize = framingStore.width;     // 500 px Breite und Höhe
const containerFovDeg = framingStore.fov;     // 2° über die gesamte Breite/Höhe
...
const x = ref(containerSize / 2 - width.value / 2);
const y = ref(containerSize / 2 - height.value / 2);
...
function calcCameraFov() {
    ...
    width.value = fovPxX;  // Breite des verschiebbaren Elements
    height.value = fovPxY; // Höhe des verschiebbaren Elements
...
function onDrag(e) {
  ...
  if (x.value > containerSize - width.value) x.value = containerSize - width.value;
  if (y.value > containerSize - height.value) y.value = containerSize - height.value;
...
function calculateRaDec() {
  // Mitte des Targets
  const targetCenterX = x.value + width.value / 2;
  const targetCenterY = y.value + height.value / 2;
